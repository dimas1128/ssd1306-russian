...
// ... ваш существующий код в ssd1306.c ...

// ===== В КОНЦЕ ФАЙЛА, ПОСЛЕ ВСЕХ ФУНКЦИЙ, НО ПЕРЕД ПОСЛЕДНЕЙ } =====

/**
 * @brief   SSD1306 Draw character with CP1251 support (русские буквы)
 *
 * @param   char character
 *
 * @return  uint8_t
 */
uint8_t SSD1306_DrawCharCP1251(char character)
{
    uint8_t i = 0;
    uint8_t index;
    ...
    // ASCII (32-127)
    if ((unsigned char)character >= 32 && (unsigned char)character <= 127) {
        index = (unsigned char)character - 32;
    }
    // Кириллица CP1251 (192-255)
    else if ((unsigned char)character >= 192 && (unsigned char)character <= 255) {
        index = (unsigned char)character - 96; // 192 -> 96, 193 -> 97, ...
    }
    else {
        index = 0;
    }
    ...
    while (i < CHARS_COLS_LENGTH) {
        cacheMemLcd[_counter++] = pgm_read_byte(&FONTS[index][i++]);
    }
    _counter++;
...
void SSD1306_DrawStringCP1251(char *str)
{
    int i = 0;
    while (str[i] != '\0') {
        SSD1306_DrawCharCP1251(str[i++]);
    }
}